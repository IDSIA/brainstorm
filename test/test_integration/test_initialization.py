...
def test_initialize_weightwise(net):
    net.initialize(7,
                   ...
                   OutputLayer={'bias': 4})
    assert np.all(net.buffer.forward.FullyConnectedLayer_1.parameters.W == 1)
    assert np.all(net.buffer.forward.FullyConnectedLayer_1.parameters.bias == 7)
    assert np.all(net.buffer.forward.FullyConnectedLayer_2.parameters.W == 2)
    assert np.all(net.buffer.forward.FullyConnectedLayer_2.parameters.bias == 3)
...
def test_initialize_with_array(net):
    ...
    assert np.all(net.buffer.forward.FullyConnectedLayer_1.parameters.bias ==
                  [1, 2])
    assert np.all(net.buffer.forward.FullyConnectedLayer_2.parameters.bias ==
                  [3, 4, 5])
