...
def create_buffer_views_from_layout(layout, buffers, context_size):
    if '@slice' in layout:
        start, stop = layout['@slice']
        shape = layout['@shape']
        t_start = context_size - layout.get('@context_size', 0)
...
class BufferManager(object):
    ...
    def get_context(self):
        ...
        self.handler.copy_to(context, timed_buffer[t - self.max_context_size:t])
        return context

    def apply_context(self, context):
        timed_buffer = self.forward_buffers[2]
        context_slice = timed_buffer[:self.max_context_size]
