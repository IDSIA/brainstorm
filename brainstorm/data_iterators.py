...
from brainstorm.handlers._cpuop import random_crop
...
class AddGaussianNoise(DataIterator, Seedable):
    """
    Adds Gaussian noise to data generated by another iterator. Only Numpy
    data is supported,
    ...
    def __init__(self, iter, std_dict, mean_dict=None, seed=None):
        ...
        for key in std_dict.keys():
            ...
            assert isinstance(iter.data[key], np.ndarray), \
...
class Flip(DataIterator, Seedable):
    """
    Randomly flip images horizontally. Only Numpy data is supported.
    ...
    def __init__(self, iter, prob_dict=None, seed=None):
        ...
        for key in prob_dict.keys():
            assert key in iter.data.keys(), "key {} is not present in " \
                                            "iterator. Available keys: {" \
                                            "}".format(key, iter.data.keys())
            assert 0.0 <= prob_dict[key] <= 1.0, "Invalid probability"
            assert isinstance(iter.data[key], np.ndarray), \
                "data with name {} is not a numpy.ndarray".format(key)
...
class Pad(DataIterator, Seedable):
    """
    Pads images equally on all sides. Only 5D Numpy data is supported.
    ...
    def __init__(self, iter, size_dict, value_dict=None, seed=None):
        ...
        if value_dict is not None:
            assert set(size_dict.keys()) == set(value_dict.keys()), \
                "padding sizes and values must be provided for " \
                "the same data names"
        for key in size_dict.keys():
            ...
            assert isinstance(iter.data[key], np.ndarray), \
                "data with name {} is not a numpy.ndarray".format(key)
            assert len(iter.data[key].shape) == 5, "Only 5D data is supported"
...
class RandomCrop(DataIterator, Seedable):
    """
    Randomly crops image data. Only 5D Numpy data is supported.
    ...
    def __init__(self, iter, shape_dict, seed=None):
        ...
        for key, val in shape_dict.items():
            ...
            assert len(data_shape) == 5, "Only 5D data is supported"
            assert data_shape[0] == 1, "Only static inputs are supported"
            assert 0 < val[0] < data_shape[3], "Invalid crop height"
            assert 0 < val[1] < data_shape[4], "Invalid crop width"
    ...
    def __call__(self, handler, verbose=False):
        for data in self.iter(handler, verbose=verbose):
            for name in self.shape_dict.keys():
                ...
                random_crop(data[name], crop_h, crop_w, row_indices,
                            col_indices, cropped)
                data[name] = cropped
            yield data
