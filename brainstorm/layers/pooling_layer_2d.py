...
class PoolingLayer2DImpl(LayerBaseImpl):
    ...
    def _setup_hyperparameters(self):
        assert 'num_filters' in self.kwargs, "num_filters must be specified " \
                                             " for PoolingLayer2D"
        assert 'window_size' in self.kwargs, "window_size must be specified " \
                                             "for PoolingLayer2D"
    ...
    def _get_output_shapes(self):
        ...
        output_height = ((in_shape[1] + 2 * pad - window_size[0]) //
                         stride[0]) + 1
        output_width = ((in_shape[2] + 2 * pad - window_size[1]) //
                        stride[1]) + 1
    ...
    def forward_pass(self, forward_buffers, training_pass=True):
        ...
        # calculate outputs
        _h.pool2d_forward_batch(flat_inputs, self.window_size, flat_outputs,
                                self.pad, self.stride, flat_argmax)
    ...
    def backward_pass(self, forward_buffers, backward_buffers):
        ...
        _h.pool2d_backward_batch(flat_inputs, self.window_size,
                                 flat_outputs, self.pad, self.stride,
